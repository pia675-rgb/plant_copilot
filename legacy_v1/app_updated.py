...
from report_4d import build_4d_pdf, payload_from_query  # noqa: E402
...
/* 매뉴얼/이력 항목 공통 (순수 HTML 패널 내부) */
.mi{padding:14px 18px 13px 18px}
.mi .item-title{font-size:13.5px;font-weight:600;line-height:1.45;margin:0 0 5px 0;color:var(--ink)}
.mi .item-body{font-size:12.5px;line-height:1.65;color:var(--ink-2);margin:0}
.mi-foot{margin-top:10px;display:flex;gap:6px;align-items:center;flex-wrap:wrap}
.mi-sep{
...
# 다크 모드 (사이드바 토글과 동기화)
if st.session_state.get("dark_toggle", False):
    st.markdown("""<style>
:root{
  --bg:#0b1220;
  --panel:#111827;
  ...
  --shadow-lg:0 4px 6px rgba(0,0,0,.35), 0 10px 24px rgba(0,0,0,.3);
}
.stApp{background:var(--bg)}
section[data-testid="stSidebar"]{
...
section[data-testid="stSidebar"] input,
section[data-testid="stSidebar"] div[data-baseweb="select"]>div{
  background:#0b1220 !important;border-color:var(--line) !important;color:var(--ink) !important}
section[data-testid="stSidebar"] .stButton>button{
  background:#0b1220 !important;border-color:var(--line) !important;color:var(--ink-2) !important}
section[data-testid="stSidebar"] .stButton>button:hover{
  background:var(--accent-soft) !important;border-color:var(--accent-line) !important;color:var(--accent) !important}
.stButton>button{
  background:#1e293b !important;border-color:var(--line) !important;color:var(--ink) !important}
.stButton>button:hover{
  background:#334155 !important;border-color:#475569 !important}
.stButton>button[kind="primary"]{
  background:linear-gradient(180deg,#0e7490 0%,#155e75 100%) !important;
  border-color:#155e75 !important;color:#fff !important}
.tagcard,.panel,.diffbar,div[data-testid="stExpander"]{
  background:var(--panel) !important;border-color:var(--line) !important}
.panel-head{background:linear-gradient(180deg,#0f172a 0%,#111827 100%) !important}
.chip{background:#1e293b !important;color:var(--ink-2) !important;border-color:var(--line) !important}
.badge{background:#1e293b !important;border-color:var(--line) !important;color:var(--muted) !important}
.step{background:var(--panel) !important;border-color:var(--line) !important}
.locbar{background:linear-gradient(135deg,#020617 0%,#0f172a 100%) !important}
.hdr .stamp{background:#1e293b !important;color:var(--faint) !important}
div[data-testid="stExpander"] input,
div[data-testid="stExpander"] textarea,
div[data-testid="stExpander"] div[data-baseweb="input"],
div[data-testid="stExpander"] div[data-baseweb="select"]>div{
  background:#0b1220 !important;border-color:var(--line) !important;color:var(--ink) !important}
/* 사이드바 브랜딩 다크 */
.pmc-brand{border-color:var(--line) !important}
.pmc-brand .pmc-title{color:var(--ink) !important}
.pmc-brand .pmc-sub{color:var(--faint) !important}
...
def delete_feedback(wo_no):
    """작업번호(wo_no)에 해당하는 이력을 삭제한다."""
    data = json.load(open(HIST, encoding="utf-8"))
    new_data = [r for r in data if r.get("wo_no") != wo_no]
...
with st.sidebar:
    st.markdown("""
    <div class="pmc-brand" style="padding:2px 0 12px;border-bottom:1px solid var(--line);margin-bottom:12px">
      <div class="pmc-title" style="font:700 14px 'Noto Sans KR',sans-serif;color:var(--ink);letter-spacing:-.02em">PMC</div>
      <div class="pmc-sub" style="font:500 11px 'IBM Plex Mono',monospace;color:var(--faint);margin-top:2px">Plant Maintenance Copilot</div>
    </div>
    """, unsafe_allow_html=True)

    st.toggle("다크 모드", key="dark_toggle")
...
# ── 4D 트러블 리포트 ────────────────────────────────────────
_adv = st.session_state.get("advice")
_advice_out = None
if _adv and len(_adv) >= 2:
    _advice_out = _adv[1]

_pdf_payload = payload_from_query(
    tag=tag, alarm=alarm, code=code, inst_row=inst, answer=a,
    tech=st.session_state.get("f_tech_cache", ""),
    advice_out=_advice_out)
_pdf_bytes = build_4d_pdf(_pdf_payload)
st.download_button(
    label="4D 트러블 리포트 (PDF)",
    data=_pdf_bytes,
    file_name="4D_Trouble_Report_%s.pdf" % tag.replace("/", "-"),
    mime="application/pdf",
    help="현재 조회 결과를 A4 4D 양식 PDF로 저장합니다.",
...
with left:
    ...
    if not a["manual"]:
        ...
        # 정비 이력과 동일: 순수 HTML 한 덩어리로 단일 패널 구성
        h = ['<div class="panel">']
        viewable = []
        for n, m in enumerate(a["manual"]):
            ...
            if has:
                viewable.append((m, fn, pg, sec or ""))
        ...
        # 원문 보기 — 패널 바로 아래 작은 버튼 (레이아웃 깨지지 않음)
        if viewable:
            cols = st.columns(min(len(viewable), 4))
            for idx, (m, fn, pg, sec) in enumerate(viewable):
                label = "p.%d" % pg
                if cols[idx % len(cols)].button(
                        label, key="pv_" + m["id"],
                        help="%s 원문 보기" % (m["name"] or m["id"]),
                        use_container_width=True):
                    st.session_state["pageview"] = (fn, pg, sec, m["id"])
                    st.rerun()
...
with right:
    ...
    if not a["history"]:
        ...
        # 이력 삭제 (테스트/시연용으로 쌓인 불필요 건 정리)
        with st.expander("이력 삭제", expanded=False):
            wo_options = [r["wo_no"] for r in a["history"]]
            del_wo = st.selectbox(
                "삭제할 작업번호",
                wo_options,
                key="del_wo_sel",
                help="이 태그에 표시된 이력만 목록에 나타납니다.")
            # 선택 건 요약
            sel = next((r for r in a["history"] if r["wo_no"] == del_wo), None)
            if sel:
                st.caption(
                    "%s · %s · %s"
                    % (sel.get("date", ""),
                       (sel.get("root_cause") or "")[:60],
                       sel.get("match", "")))
            confirm = st.checkbox("삭제에 동의합니다", key="del_wo_confirm")
            if st.button("선택 이력 삭제", type="secondary",
                         disabled=not confirm, key="del_wo_btn"):
                if delete_feedback(del_wo):
                    st.cache_resource.clear()
                    st.session_state.pop("del_wo_confirm", None)
                    st.success("%s 이력을 삭제했습니다." % del_wo)
                    st.rerun()
                else:
                    st.error("해당 이력을 찾지 못했습니다.")
