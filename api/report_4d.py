...
def build_4d_pdf(payload: dict) -> bytes:
    ...
    # ---- 레이아웃 상수 (A4 mm 기준) ----
    M = 12 * mm              # 페이지 좌우 여백
    SPINE = 16 * mm          # D1~D4 왼쪽 화살표 칸 (원과 카드 사이 여백 확보)
    card_l = M + SPINE
    card_r = W - M
    FOOTER_H = 16 * mm
    ...
    headers = [
        ...
        ("Maker / Model", mm_val),
        # 판넬만으로는 현장에서 어디로 가야 하는지 알 수 없다. 배치도
        # 그리드를 같이 찍으면 이 한 줄로 이동이 끝난다.
        ("판넬 / 단자", "%s%s / %s" % (
            payload.get("panel") or "-",
            (" (%s)" % payload["panel_grid"]) if payload.get("panel_grid") else "",
